...
const ROWS = [
  ...
  { label: "שפות", before: "תלוי מי במשמרת", after: "עברית ואנגלית, אוטומטית" },
...
export function Comparison() {
  return (
    <section className="container-page py-28 md:py-40">
      <Reveal className="overflow-hidden rounded-tiles bg-silver text-void">
        <div className="grid grid-cols-1 gap-10 p-8 md:p-14 lg:grid-cols-12">
          <div className="flex flex-col gap-5 lg:col-span-4">
            <span className="mono-label text-void/60">לפני ואחרי</span>
            <h2 className="display-serif text-balance text-[40px] leading-[1.02] sm:text-5xl">
              מה משתנה
              <br />
              <span className="italic">ביום הראשון.</span>
            </h2>
            <p className="max-w-xs text-body text-void/70">
              לא מחליפים את הצוות — משחררים אותו מהטלפון.
            </p>
          </div>

          <div className="lg:col-span-8">
            <div className="hidden grid-cols-[140px_1fr_1fr] gap-x-4 border-b border-void/15 pb-3 sm:grid">
              <span className="mono-label text-void/50">&nbsp;</span>
              <span className="mono-label text-void/50">רצפציה מסורתית</span>
              <span className="mono-label text-void">קולי AI</span>
            </div>
            <ul>
              {ROWS.map((r) => (
                <li
                  key={r.label}
                  className="grid grid-cols-1 gap-y-2 border-b border-void/10 py-4 last:border-0 sm:grid-cols-[140px_1fr_1fr] sm:items-start sm:gap-x-4"
                >
                  <span className="mono-label text-void/70 sm:font-sans sm:normal-case sm:tracking-normal sm:text-body-sm sm:font-medium sm:text-void">
                    {r.label}
                  </span>
                  <span className="flex items-start gap-2 text-body-sm text-void/55">
